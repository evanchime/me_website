...
class ExperienceViewTests(TestCase):
    ...
    def test_experience_view_never_cache_decorator(self):
        """Test that experience view has never_cache decorator applied."""
        response = self.client.get(self.experience_url)
    ...
    def test_experience_view_post_method_allowed(self):
        """Test that experience view handles POST requests."""
    ...
    def test_experience_url_name(self):
        """Test that the experience URL name resolves correctly."""
        url = reverse('experience')
        self.assertEqual(url, '/experience/')
    
    def test_experience_view_with_query_parameters(self):
        """Test that experience view handles query parameters gracefully."""
        response = self.client.get(self.experience_url + '?company=tech&role=developer')
        self.assertEqual(response.status_code, 200)
    
    def test_experience_view_with_invalid_query_parameters(self):
        """Test that experience view handles invalid query parameters."""
        response = self.client.get(self.experience_url + '?<script>alert("xss")</script>')
        self.assertEqual(response.status_code, 200)
    
    def test_experience_view_response_headers(self):
...
class ExperienceURLTests(TestCase):
    ...
    def test_experience_url_reverse(self):
        """Test that experience URL name reverses correctly."""
        url = reverse('experience')
        self.assertEqual(url, '/experience/')
...
class ExperienceIntegrationTests(TestCase):
    ...
    def test_experience_page_accessibility(self):
        """Test basic accessibility of experience page."""
