...
def lambda_handler(event, context):
    ...
    This handler uses the single-user rotation scheme to rotate an RDS 
    PostgreSQL user credential. This rotation scheme logs into the 
    ...
    following format:
    {
        ...
        'port': <optional: if not specified, default to port 5432>
    }
...
def create_secret(service_client, arn, token):
    ...
    passed in token. If one does not exist, it will generate a new secret 
    and put it with the passed in token.
    ...
    try:
        ...
        logger.info(
            "createSecret: Successfully retrieved secret for %s." % arn
        )
...
def set_secret(service_client, arn, token):
    ...
    the AWSPENDING password as the user password in the database. 
    ...
    if conn:
        ...
        logger.info(
            ...
            "PostgreSQL DB for secret arn %s." % arn
        )
        return
    ...
    if current_dict['host'] != pending_dict['host']:
        logger.error(
            "setSecret: Attempting to modify user for host %s other than "
            "current host %s" % (pending_dict['host'], current_dict['host'])
        )
        raise ValueError(
    ...
    if not conn and previous_dict:
        ...
        if previous_dict['username'] != pending_dict['username']:
            logger.error(
                "setSecret: Attempting to modify user %s other than previous "
                "valid user %s" 
        ...
        if previous_dict['host'] != pending_dict['host']:
            logger.error(
                "setSecret: Attempting to modify user for host %s other than "
                "previous valid host %s" 
                % (pending_dict['host'], previous_dict['host'])
            )
            raise ValueError(
                "Attempting to modify user for host %s other than previous "
                "valid host %s" % (pending_dict['host'], previous_dict['host'])
            )

    # If we still don't have a connection, raise a ValueError
    if not conn:
        logger.error(
            "setSecret: Unable to log into database with previous, current, or "
...
def test_secret(service_client, arn, token):
    ...
    if conn:
        ...
        # Uncomment/modify the below lines to tailor these validations to 
        # your needs
        try:
            with conn.cursor() as cur:
        ...
        logger.info(
            "testSecret: Successfully signed into PostgreSQL DB with "
        ...
        raise ValueError(
            "Unable to log into database with pending secret of secret ARN %s" 
            % arn
        )
...
def get_connection(secret_dict):
    ...
    This helper function uses connectivity information from the secret 
    dictionary to initiate connection attempt(s) to the database. 
    Will attempt a fallback, non-SSL connection when initial connection 
    fails using SSL and fall_back is True.

    Args:
        secret_dict (dict): The Secret Dictionary

    Returns:
        Connection: The pgdb.Connection object if successful. 
        None otherwise
...
def get_ssl_config(secret_dict):
    """Gets the desired SSL and fall back behavior using a secret 
    dictionary

    This helper function uses the existance and value the 'ssl' key in a 
    secret dictionary to determine desired SSL connectivity configuration. 
    ...
    Returns:
        Tuple(use_ssl, fall_back): SSL configuration
            - use_ssl (bool): Flag indicating if an SSL connection 
            should be attempted
            - fall_back (bool): Flag indicating if non-SSL connection 
            should be attempted if SSL connection fails
    ...
    if isinstance(secret_dict['ssl'], str):
        ...
        if ssl == "true":
            ...
            # Invalid string value, default to True for both SSL and 
            # fall_back mode
            return True, True
...
def connect_and_authenticate(secret_dict, port, dbname, use_ssl):
    ...
    This helper function tries to connect to the database using 
    connectivity info passed in. If successful, it returns the connection, 
    else None
    ...
    Args:
        ...
        - use_ssl (bool): Flag indicating whether connection should use 
        SSL/TLS

    Returns:
        Connection: The pymongo.database.Database object if successful. 
        None otherwise
...
def get_secret_dict(service_client, arn, stage, token=None):
    ...
    This helper function gets credentials for the arn and stage passed 
    in and returns the dictionary by parsing the JSON string
    ...
    Args:
        ...
        token (string): The ClientRequestToken associated with the 
        secret version, or None if no validation is desired
    ...
    if (
        'engine' not in secret_dict 
        or secret_dict['engine'] not in supported_engines
    ):
        raise KeyError(
            "Database engine must be set to 'postgres' in order to use this "
            "rotation lambda"
        )
...
def get_environment_bool(variable_name, default_value):
    ...
    Args:
        ...
        default_value (bool): The result will fallback to the 
        default_value when the environment variable with the given name 
        doesn't exist.

    Returns:
        bool: True when the content of environment variable contains 
        either 'true', '1', 'y' or 'yes'
    """
...
def get_random_password(service_client):
    """ Generates a random new password. Generator loads parameters that 
    affects the content of the resulting password from the environment
    variables. When environment variable is missing sensible defaults 
    are chosen.
